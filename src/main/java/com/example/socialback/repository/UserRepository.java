...
public interface UserRepository extends Neo4jRepository<User, Long> {
    ...
    @Query("MATCH (user:`User`) WHERE user.username = $username RETURN user{.createdAt, .email, .firstName, .lastName, .password, .profilePicture, .roles, .updatedAt, .username, __nodeLabels__: labels(user), __internalNeo4jId__: ID(user), __elementId__: ID(user)}")
